...
fn create_multiple_individuals(individual: &Individual, max_routes_range: (i32, i32)) -> Option<Vec<Individual>> {
    ...
    let route_groups_distances = indexed_medoids
        ...
        .map(|(outer_idx, outer_medoid)| {
            ...
            let shuffle_count = individual.random.uniform_int(2, (route_distances.len() as i32 / 4).max(2)) as usize;
            route_distances.partial_shuffle(&mut individual.random.get_rng(), shuffle_count);
    ...
    let individuals = route_groups_distances
        ...
        .map(|(outer_idx, route_group_distance)| {
            let group_size = individual.random.uniform_int(max_routes_range.0, max_routes_range.1) as usize;
